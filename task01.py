...
class BloomFilter:
    ...
    def add(self, item):
        ...
        :param item: Елемент для додавання (рядок)
        """
    ...
    def contains(self, item):
        ...
        :param item: Елемент для перевірки (рядок)
        :return: True, якщо елемент може бути у фільтрі; False, якщо елемента точно немає
        """
        for i in range(self.num_hashes):
            index = mmh3.hash(item, i) % self.size
